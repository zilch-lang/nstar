...
parseCjz :: MonadParser m => m (Located Instruction)
parseCjz =
  located $
    lexeme (parseSymbol Cjz)
      *> ( CJZ <$> MP.choice [located (RegE <$> parseRegister), located (ImmE <$> parseImmediate)]
             <*> (lexeme (parseSymbol Comma) *> parseLabel)
             <*> (lexeme (parseSymbol Comma) *> parseLabel)
         )

parseCjnz :: MonadParser m => m (Located Instruction)
parseCjnz =
  located $
    lexeme (parseSymbol Cjnz)
      *> ( CJNZ <$> MP.choice [located (RegE <$> parseRegister), located (ImmE <$> parseImmediate)]
             <*> (lexeme (parseSymbol Comma) *> parseLabel)
             <*> (lexeme (parseSymbol Comma) *> parseLabel)
         )

parseCjl :: MonadParser m => m (Located Instruction)
parseCjl =
  located $
    lexeme (parseSymbol Cjl)
      *> ( CJL <$> MP.choice [located (RegE <$> parseRegister), located (ImmE <$> parseImmediate)]
             <*> (lexeme (parseSymbol Comma) *> MP.choice [located (RegE <$> parseRegister), located (ImmE <$> parseImmediate)])
             <*> (lexeme (parseSymbol Comma) *> parseLabel)
             <*> (lexeme (parseSymbol Comma) *> parseLabel)
         )

parseCjge :: MonadParser m => m (Located Instruction)
parseCjge =
  located $
...
parseCje :: MonadParser m => m (Located Instruction)
parseCje =
  located $
    lexeme (parseSymbol Cje)
      *> ( CJE <$> MP.choice [located (RegE <$> parseRegister), located (ImmE <$> parseImmediate)]
             <*> (lexeme (parseSymbol Comma) *> MP.choice [located (RegE <$> parseRegister), located (ImmE <$> parseImmediate)])
             <*> (lexeme (parseSymbol Comma) *> parseLabel)
             <*> (lexeme (parseSymbol Comma) *> parseLabel)
         )

parseCjne :: MonadParser m => m (Located Instruction)
parseCjne =
  located $
    lexeme (parseSymbol Cjne)
      *> ( CJNE <$> MP.choice [located (RegE <$> parseRegister), located (ImmE <$> parseImmediate)]
             <*> (lexeme (parseSymbol Comma) *> MP.choice [located (RegE <$> parseRegister), located (ImmE <$> parseImmediate)])
             <*> (lexeme (parseSymbol Comma) *> parseLabel)
             <*> (lexeme (parseSymbol Comma) *> parseLabel)
         )

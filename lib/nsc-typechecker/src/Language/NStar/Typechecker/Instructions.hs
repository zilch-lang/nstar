...
tc_shiftl (x :@ p1) (y :@ p2) (r :@ p3) p4 = do
  {-
     Ξ; Γ; χ; σ; ε ⊢ᵀ x : uN         r is a register              r ≠ ε
  ─────────────────────────────────────────────────────────────────────────
           Ξ; Γ; χ; σ; ε ⊢ᴵ shiftl x, y, r ⊣ χ, r : uN; σ; ε

     Ξ; Γ; χ; σ; ε ⊢ᵀ x : sN         r is a register              r ≠ ε
  ─────────────────────────────────────────────────────────────────────────
           Ξ; Γ; χ; σ; ε ⊢ᴵ shiftl x, y, r ⊣ χ, r : sN; σ; ε
  -}
  ...
  pure (TC.SHIFTL x (y :@ p2) (r :@ p3))

tc_shiftr :: (?tcFlags :: TypecheckerFlags) => Located Expr -> Located Integer -> Located Register -> Position -> Typechecker TC.TypedInstruction
tc_shiftr (x :@ p1) (y :@ p2) (r :@ p3) p4 = do
  {-
     Ξ; Γ; χ; σ; ε ⊢ᵀ x : uN         r is a register              r ≠ ε
  ─────────────────────────────────────────────────────────────────────────
           Ξ; Γ; χ; σ; ε ⊢ᴵ shiftr x, y, r ⊣ χ, r : uN; σ; ε

     Ξ; Γ; χ; σ; ε ⊢ᵀ x : sN         r is a register              r ≠ ε
  ─────────────────────────────────────────────────────────────────────────
           Ξ; Γ; χ; σ; ε ⊢ᴵ shiftr x, y, r ⊣ χ, r : sN; σ; ε
  -}
  ...
  pure (TC.SHIFTR x (y :@ p2) (r :@ p3))

tc_mul :: (?tcFlags :: TypecheckerFlags) => Located Expr -> Located Expr -> Located Register -> Position -> Typechecker TC.TypedInstruction
tc_mul (x :@ p1) (y :@ p2) (r :@ p3) p4 = do
  {-
     Ξ; Γ; χ; σ; ε ⊢ᵀ x : uN                Ξ; Γ; χ; σ; ε ⊢ᵀ y : uN
               r is a register              r ≠ ε
  ──────────────────────────────────────────────────────────────────────
           Ξ; Γ; χ; σ; ε ⊢ᴵ mul x, y, r ⊣ χ, r : uN; σ; ε

     Ξ; Γ; χ; σ; ε ⊢ᵀ x : sN                Ξ; Γ; χ; σ; ε ⊢ᵀ y : sN
               r is a register              r ≠ ε
  ──────────────────────────────────────────────────────────────────────
           Ξ; Γ; χ; σ; ε ⊢ᴵ mul x, y, r ⊣ χ, r : sN; σ; ε
  -}
...
tc_cmvle :: (?tcFlags :: TypecheckerFlags) => Located Expr -> Located Expr -> Located Expr -> Located Expr -> Located Register -> Position -> Typechecker TC.TypedInstruction
tc_cmvle (a :@ p1) (b :@ p2) (c :@ p3) (d :@ p4) (r :@ p5) p6 = do
  {-
     Ξ; Γ; χ; σ; ε ⊢ᵀ a : τ₁          Ξ; Γ; χ; σ; ε ⊢ᵀ b : τ₁
                      r ≠ ε ∧ c ≠ ε ∧ d ≠ ε
     Ξ; Γ; χ; σ; ε ⊢ᵀ c : τ₂           Ξ; Γ; χ; σ; ε ⊢ᵀ d : τ₂
  ─────────────────────────────────────────────────────────────────
     Ξ; Γ; χ; σ; ε ⊢ᴵ cmvle a, b, c, d, r ⊣ Ξ; Γ; χ, r : τ₂; σ; ε
  -}
  ...
  pure (TC.CMVLE a b c d (r :@ p5))

tc_cmve :: (?tcFlags :: TypecheckerFlags) => Located Expr -> Located Expr -> Located Expr -> Located Expr -> Located Register -> Position -> Typechecker TC.TypedInstruction
tc_cmve (a :@ p1) (b :@ p2) (c :@ p3) (d :@ p4) (r :@ p5) p6 = do
  {-
     Ξ; Γ; χ; σ; ε ⊢ᵀ a : τ₁          Ξ; Γ; χ; σ; ε ⊢ᵀ b : τ₁
                      r ≠ ε ∧ c ≠ ε ∧ d ≠ ε
     Ξ; Γ; χ; σ; ε ⊢ᵀ c : τ₂           Ξ; Γ; χ; σ; ε ⊢ᵀ d : τ₂
  ─────────────────────────────────────────────────────────────────
     Ξ; Γ; χ; σ; ε ⊢ᴵ cmve a, b, c, d, r ⊣ Ξ; Γ; χ, r : τ₂; σ; ε
  -}
...
tc_cjl :: (?tcFlags :: TypecheckerFlags) => Located Expr -> Located Expr -> Located Expr -> Located Expr -> Position -> Typechecker TC.TypedInstruction
tc_cjl (a :@ p1) (b :@ p2) (lt :@ p3) (lf :@ p4) p5 = do
  {-
     Ξ; Γ; χ; σ; ε ⊢ᵀ lᵗ<v⃗> : ∀().{ χ₁′ | σ → ε }     χ ∼ χ₁′
     Ξ; Γ; χ; σ; ε ⊢ᵀ lᶠ<v⃗> : ∀().{ χ₂′ | σ → ε }     χ ∼ χ₂′
     Ξ; Γ; χ; σ; ε ⊢ᵀ a : τ₁         Ξ; Γ; χ; σ; ε ⊢ᵀ b : τ₁
  ─────────────────────────────────────────────────────────────── conditional jump
        Ξ; Γ; χ; σ; ε ⊢ᴵ cjl a, b, lᵗ<v⃗>, lᶠ<v⃗> ⊣ χ; σ; ε
  -}
...
tc_cjle :: (?tcFlags :: TypecheckerFlags) => Located Expr -> Located Expr -> Located Expr -> Located Expr -> Position -> Typechecker TC.TypedInstruction
tc_cjle (a :@ p1) (b :@ p2) (lt :@ p3) (lf :@ p4) p5 = do
  {-
     Ξ; Γ; χ; σ; ε ⊢ᵀ lᵗ<v⃗> : ∀().{ χ₁′ | σ → ε }     χ ∼ χ₁′
     Ξ; Γ; χ; σ; ε ⊢ᵀ lᶠ<v⃗> : ∀().{ χ₂′ | σ → ε }     χ ∼ χ₂′
     Ξ; Γ; χ; σ; ε ⊢ᵀ a : τ₁         Ξ; Γ; χ; σ; ε ⊢ᵀ b : τ₁
  ─────────────────────────────────────────────────────────────── conditional jump
        Ξ; Γ; χ; σ; ε ⊢ᴵ cjle a, b, lᵗ<v⃗>, lᶠ<v⃗> ⊣ χ; σ; ε
  -}

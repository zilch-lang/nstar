...
import Error.Diagnose (Marker (..), Note (..), Report(Err, Warn))
import Language.NStar.Syntax.Core (Register)
import Language.NStar.Typechecker.Core (Kind, Type (RecordT, VarT))
...
fromTypecheckWarning _ = Warn Nothing "" [] []
...
fromTypecheckError (RecordUnify err (m1 :@ p1) (m2 :@ p2)) = fromTypecheckError err <> Warn Nothing "\n" [] [] <> recordValuesDoNotUnify (m1, p1) (m2, p2)
--                                                                                      ^^^^^^^^^^^^^^^
...
fromTypecheckError (CannotJumpBecauseOf p err) = cannotJumpAt p <> Warn Nothing "\n" [] [] <> fromTypecheckError err
--                                                                  ^^^^^^^^^^^^^^^
-- This is just to insert a newline between error

...
import Error.Diagnose (Report, Marker(..), err, warn)
...
uncoercibleTypes (t1, p1) (t2, p2) =
  err ("Type '" <> show (pretty t1) <> "' cannot be coerced to '" <> show (pretty t2) <> "'.")
    [ (p1, This "") ]
...
retWithoutReturnAddress p r ctx =
  err ("The `ret` instruction expects a return address to be stored in the continuation register, but did not find one.")
...
infiniteType (ty, p1) (var, p2) =
  err ("Cannot create the infinite type from the relation '" <> show (pretty ty) <> " ~ " <> Text.unpack var <> "'.")
...
recordDomainsDoNotSubset (m1, p1) (m2, p2) =
  err ("All keys in '" <> show (pretty m1) <> "' are not present in '" <> show (pretty m2) <> "'")
    [ (p1, This "") ]
    []
...
instance Pretty (Map (Located Register) (Located Type)) where
  pretty = encloseSep lbrace rbrace comma . fmap prettyBind . Map.toList
    where prettyBind (r, t) = pretty r <+> colon <+> pretty t
...
recordValuesDoNotUnify (m1, p1) (m2, p2) =
  let s1 = VarT ("_s1" :@ p1) :@ p1
      ...
      e1 = VarT ("_e1" :@ p1) :@ p1
      e2 = VarT ("_e2" :@ p2) :@ p2
  in err ("Record types cannot be coerced because at least one of their common fields cannot be coerced to each other.")
...
returnAtTopLevel p =
  err ("Attempting to `ret` when no label has been crossed so far.")
    [ (p, This "") ]
    []
...
contextIsMissingOnReturnAt regs p1 p2 =
  err "Current context is missing some registers in order to return safely."
...
kindIsNotAStackKind k p1 p2 =
  err ("Kind '" <> show (pretty k) <> "' cannot be used in place of a stack kind.")
...
kindIsNotADataKind k p1 p2 =
  err ("Kind '" <> show (pretty k) <> "' was expected to be a data kind, but was found not to be one.")
...
kindIsUnsized k p1 p2 =
  err ("Kind '" <> show (pretty k) <> "' is unsized, but was expected to be sized.")
...
unboundTypeVariable v p =
  err ("Type variable '" <> Text.unpack v <> "' was not found in scope.")
...
registerNotFoundInContext r p ctx =
  err ("Register '" <> show (pretty r) <> "' was not found in the current context, or is not set.")
    [ (p, This "")
...
unknownLabel name callPos =
  err ("Trying to jump to the label '" <> Text.unpack name <> "' but it has not been found in the file.")
...
cannotInferSpecialization nbGot nbExpected p =
  err ("Cannot infer type variable specialization for the missing " <> show (nbExpected - nbGot) <> " type parameters.")
...
unsafeNotInUnsafeBlock p =
  err "Unsafe operation not enclosed in an 'unsafe' block."
...
cannotPopCodespaceAddress ty p =
  err "Cannot pop a code-space address off the stack."
...
cannotMovToDestination ty1 ty2 p1 p2 =
  err "Unable to move the source operand into the destination."
...
cannotPopIntoDestination expected sHead sTail p1 p2 =
  err "Unable to pop the stack pointer into the destination."
...
cannotUnifyKinds k1 k2 p1 p2 =
  err ("Cannot unify kinds " <> show (pretty k1) <> " and " <> show (pretty k2))
...
cannotReturnToUnknownContinuation p1 cont =
  err "Trying to jump back to a continuation hidden behind a type variable."
...
cannotOverwriteRegisterContinuation (r :@ p1) p2 =
  err "Cannot overwrite the register containing the current continuation."
...
stackIsNotBigEnough n t p =
  err ("Stack infered must be at least of length " <> show n)
...
cannotReturnToStackContinuation t p =
  err "Cannot return to a continuation stored on the stack."
...
cannotCallOnAbstractContinuation t p =
  err "Cannot call a function when the current continuation is abstract."
...
cannotDiscardContinuationFromStack p =
  err "Cannot discard the continuation stored on top of the stack."
...
cannotStoreContOnHeap r p1 p2 =
  err "Cannot move the current continuation into a heap object."
...
cannotPropagateRegister rs bs p1 p2 =
  err "Cannot propagate some of the registers because they were marked as potentially overwritten."
  ...
  where showCommaSep regs = "{" <> intercalate "," (show . pretty <$> regs) <> "}"
...
cannotReferenceFunctionPointerOnStack s n p1 p2 =
  err "Trying to reference a function pointer is disallowed for safety reasons."
...
structureOffsetMustBeACompileTimeConstant p1 p2 p3 =
  err "Trying to access a structure field with an unknown index."
...
outOfBoundsStructureAccess n s p =
  err "Out of bounds member access in structure."
